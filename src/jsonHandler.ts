export class JsonHandler {
	...
	*/
	...
	/**
	 * Checks if a JSON already exists on a filepath.
	 * @param databaseFilePath The filepath of the JSON file.
	 * @returns Whether the JSON already exists or not.
	 */
	public async jsonExists(databaseFilePath: string) {
		// eslint-disable-next-line @typescript-eslint/no-var-requires
		const fs = require('fs');
		const exists: boolean = await fs.access(databaseFilePath, fs.F_OK, (err: any) => {
			if (err) {
				return false;
			} else {
				return true;
